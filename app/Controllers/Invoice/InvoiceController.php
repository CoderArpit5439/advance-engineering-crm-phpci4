...
class InvoiceController extends ResourceController
{
    ...
    public function fetchAllInvoices()
    {
        ...
        try {
            $invoiceData = $invoiceModel->findAll();
    ...
    public function updateInvoiceDescription()
{
    $invoiceModel = new InvoiceModel();
    $inv_id = $this->request->getVar("inv_id") ?? "";
    $inv_description = $this->request->getVar("inv_description") ?? "";

    if (!$inv_id || !$inv_description) {
        return $this->response->setJSON([
            "status" => false,
            "message" => "Invoice ID or description not provided"
        ]);
    }

    // Find the invoice by ID
    $invoice = $invoiceModel->find($inv_id);
    ...
    // Update only the inv_description field
    $updateData = [
        'inv_description' => $inv_description
    ];

    try {
        $invoiceUpdate = $invoiceModel->update($inv_id, $updateData); // Update using the ID
        if ($invoiceUpdate) {
            return $this->response->setJSON([
                "status" => true,
                "message" => "Invoice description updated successfully",
                "data" => $updateData
            ]);
        } else {
            return $this->response->setJSON([
                "status" => false,
                "message" => "Failed to update invoice description"
            ]);
        }
    } catch (\Throwable $th) {
        return $this->response->setJSON([
    ...
    public function deleteInvoice()
    {
        ...
        try {
            ...
            // Delete the invoice
            $deleteInvoice = $invoiceModel->delete($invoiceId);
    
            if ($deleteInvoice) {
                // Fetch all invoices after deletion
