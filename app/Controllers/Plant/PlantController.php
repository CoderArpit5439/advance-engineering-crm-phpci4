...
class PlantController extends ResourceController
{
    ...
    public function fetchSinglePlant()
    {
        ...
        try {
            $plant_id      = $this->request->getVar('id');
            $plantModel    = new PlantModel();
            $unitModel     = new UnitModel();
            $customerModel = new CustomerModel();

            // $quotationModel = new QuotationModel();
            $singlePlant = $plantModel->where('p_id', $plant_id)->first();

            $unitList     = [];
            $customerList = [];
            // foreach ($plantList as $plant) {
            $unitDetails = $unitModel->where('u_plant_id', $plant_id)->findAll();
            if ($unitDetails) {
